...
HAS_EXAMPLE_SOURCE = (EXEMPLES_DIR / "source.xlsx").exists()
HAS_EXAMPLE_CSV = any(EXEMPLES_DIR.glob("*.csv"))
HAS_FULL_EXAMPLES = HAS_EXAMPLE_SOURCE and HAS_EXAMPLE_CSV
EXAMPLES_REASON = "Données d'exemple non disponibles dans le dépôt local"
...
from src.utils.semester import Semester, infer_semester_from_bulletins_data


@pytest.mark.skipif(not HAS_FULL_EXAMPLES, reason=EXAMPLES_REASON)
...
class TestBulletinProcessor:
    ...
    @pytest.mark.skipif(not HAS_EXAMPLE_CSV, reason=EXAMPLES_REASON)
    def test_populate_bulletins_from_csv(self):
        ...
        populate_bulletins_from_csv(bulletins, matiere_data, "Mathematiques", Semester.S2)
    ...
    def test_populate_bulletins_from_csv_semester_s1(self):
        """Test le mapping des colonnes en mode semestre 1."""
        eleves_data = [{'Élève': 'DUPONT Alice'}]
        bulletins = create_bulletins_from_source(eleves_data)
        ...
        matiere_data = [{
            ...
            'Moy. S1': '14,50',
            'H.Abs.': '1h00',
            'App. A : Appréciations': 'Bon départ au collège'
        }]
        
        populate_bulletins_from_csv(bulletins, matiere_data, "Mathematiques", Semester.S1)
...
class TestMainProcessor:
    ...
    def test_process_directory_to_json_exemples(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert data and "_metadata" in data[0]
            assert data[0]['_metadata']['semester'] == result['semester']
            
            bulletins_data = data[1:]
            assert len(bulletins_data) == result['bulletins_count']
            assert all('Nom' in item and 'Prenom' in item for item in bulletins_data)
...
class TestSemesterUtils:
    """Tests pour les utilitaires de semestre."""
    
    def test_infer_semester_from_bulletins_data(self):
        """Vérifie la détection du semestre à partir d'un JSON minimal."""
        bulletins_data = [{
            "Nom": "TEST",
            "Prenom": "Eleve",
            "Matieres": {
        ...
        assert infer_semester_from_bulletins_data(bulletins_data) == Semester.S1
        
        bulletins_data[0]["Matieres"]["Maths"]["MoyenneS2"] = 15.0
        assert infer_semester_from_bulletins_data(bulletins_data) == Semester.S2
